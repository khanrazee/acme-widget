  ...
  describe 'POST #add_to_cart' do
    context 'with valid parameters' do
      it 'adds a product to the cart' do
        expect do
          post :add_to_cart, params: { product_id: product.id, quantity: 2 }
      ...
      it 'redirects to cart_path with a notice' do
        ...
        expect(flash[:notice]).to include('was successfully added to your cart')
      end
    end

    context 'with invalid parameters' do
      it 'redirects back with an alert when quantity is less than 1' do
        post :add_to_cart, params: { product_id: product.id, quantity: 0 }
        expect(response).to redirect_to(root_path)
        expect(flash[:alert]).to eq('Quantity must be at least 1')
      end
    end
  end
  ...
  describe 'PATCH #update_quantity' do
      ...
      it 'updates the quantity of the cart item' do
        expect do
          patch :update_quantity,
                params: { id: controller.current_user.cart.id, product_id: product.id, quantity: 3 }
        end.to change {
    ...
    context 'when removing product' do
      ...
      it 'removes the product from the cart' do
        expect do
          patch :update_quantity,
                params: { id: controller.current_user.cart.id, product_id: product.id, quantity: 0 }
        end.to change { controller.current_user.cart.cart_items.count }.by(-1)
      end
