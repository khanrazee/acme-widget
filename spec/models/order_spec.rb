...
RSpec.describe Order, type: :model do
  ...
  describe '#add_items_from_cart' do
    ...
    it 'copies the correct attributes from cart items' do
      ...
      line_item1 = order.line_items.find_by(product_id: product1.id)
      line_item2 = order.line_items.find_by(product_id: product2.id)
      
      expect(line_item1.quantity).to eq(2)
      expect(line_item1.price_cents).to eq(19)
      expect(line_item2.quantity).to eq(1)
      expect(line_item2.price_cents).to eq(29)
    end
  ...
  describe '#calculate_total' do
    ...
    before do
      order.line_items.create(product: product1, quantity: 2, price: 19.99)
      order.line_items.create(product: product2, quantity: 1, price: 29.99)
    end
